...
export function createChecks(io: DoctorIO): Check[] {
  ...
  return [
    ...
    { name: "Browser Adapter", run: () => browserAdapterOutcome(io, paths.nexusDir) },
...
async function browserAdapterOutcome(io: DoctorIO, nexusDir: string): Promise<CheckOutcome> {
  const wrapper = join(io.homeDir(), ".local", "bin", "nexus-browser")
  const venvMarker = join(nexusDir, "browser-adapter", ".venv", "bin", "browser-use")
  const hasWrapper = await io.exists(wrapper)
  const hasVenv = await io.exists(venvMarker)
  if (hasWrapper && hasVenv) return ok("nexus-browser wrapper + venv present")
  if (hasWrapper || hasVenv) return skipped("partial install detected", "Re-run bash scripts/install-browser-minimal.sh to repair.")
  return skipped(
    "not installed (optional)",
    "Pick a size in TUI: Ctrl+P -> Browser Adapter, then run bash scripts/install-browser-minimal.sh.",
  )
}
